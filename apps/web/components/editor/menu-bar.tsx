...
export function MenuBar({ projectId, showLeft = true, showRight = true, toggleLeft, toggleRight, leftWidth, rightWidth, statesHeight, setLeftWidth, setRightWidth, setStatesHeight }: MenuBarProps) {
  ...
  return (
    <div className="w-full pointer-events-none sticky top-0 z-50 flex justify-center px-4 pt-[env(safe-area-inset-top,12px)]">
      <div className="w-full max-w-[440px] h-12 flex items-center justify-between px-3 rounded-2xl ios-glass border-border/50 shadow-lg backdrop-blur-2xl pointer-events-auto">
        <div className="flex items-center gap-2">
          <div className="border rounded-md p-0.5">
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button variant="ghost" className="h-8 px-2 touch-target-min">
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="flex items-center gap-1 border rounded-md p-0.5 sm:gap-2">
            ...
            <div className="sm:hidden">
              <DropdownMenu>
                ...
                <DropdownMenuContent align="end" className="w-40">
                  <div className="hidden sm:block">
                    <DropdownMenuItem onClick={() => toggleLeft?.()}>
                      <PanelLeft className="h-4 w-4 mr-2" /> {showLeft ? "Hide" : "Show"} Layers
                    </DropdownMenuItem>
                    <DropdownMenuItem onClick={() => toggleRight?.()}>
                      <PanelRight className="h-4 w-4 mr-2" /> {showRight ? "Hide" : "Show"} Inspector
                    </DropdownMenuItem>
                  </div>
                  <DropdownMenuSeparator />
