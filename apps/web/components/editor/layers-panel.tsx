...
export function LayersPanel() {
  ...
  return (
    <div className="flex flex-col gap-1 min-h-0" data-tour-id="layers-panel">
      <div className="flex items-center justify-between px-2 py-2 shrink-0">
        <div className="font-semibold text-lg text-muted-foreground/80 uppercase tracking-tight">Layers</div>
        {uploadStatus && (
        ...
        <input
          ...
          className="hidden"
          aria-label="Upload video or GIF"
      ...
      <div className="flex-1 min-h-0 overflow-hidden">
        <div className="ios-inset-group mx-0! my-0! h-full flex flex-col">
          <div
          ...
          >
            <div
              className={cn(
                "ios-list-item cursor-pointer font-medium select-none transition-all ios-active-scale",
                selectedId === '__root__' ? "bg-muted" : "bg-card"
              )}
            ...
            >
              <div className="flex items-center gap-1 flex-1">
                {layers.length > 0 ? (
                  <button
                    ...
                    className="shrink-0 hover:bg-accent/50 rounded p-0.5"
                    title={rootCollapsed ? "Expand root layer" : "Collapse root layer"}
                    aria-label={rootCollapsed ? "Expand root layer" : "Collapse root layer"}
                  >
                    {rootCollapsed ? (
                      <ChevronRight className="h-3 w-3" />
                    ) : (
              ...
              <div className="text-ios-blue">
                {selectedId === '__root__' && <Check className="h-4 w-4" />}
              </div>
            </div>
            {!rootCollapsed && (
              <>
                {layers.length === 0 && (
                  <div className="py-2 pl-6 text-muted-foreground">No layers yet</div>
                )}
