...
import { ArrowRight, Eye, Settings, Check } from "lucide-react";
...
export function StatesPanel() {
  ...
  return (
    <div className="flex flex-col gap-1 min-h-0" data-tour-id="states-panel">
      <div className="flex items-center justify-between px-2 py-2 shrink-0">
        <div className="font-semibold text-lg text-muted-foreground/80 uppercase tracking-tight">States</div>
        <div className="flex items-center gap-2">
          {/* ... dialog and settings buttons ... */}
                  ...
                  <div>
                    ...
                    <div className="space-y-4">
                      {Object.entries(overridesByLayer).map(([layerId, stateData]) => (
                        <div key={layerId} className="border rounded-xl p-4 bg-muted/30">
          ...
          {!gyroEnabled && (
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end" className="w-64 p-2 ios-glass rounded-2xl">
      ...
      <div className="flex-1 overflow-hidden">
        <div className="ios-inset-group mx-0! my-0!">
          <div className="flex-1 overflow-auto">
            {/* base state */}
            <button
              type="button"
              className={`ios-list-item w-full text-left transition-all ios-active-scale ${active === 'Base State' ? 'bg-muted' : 'bg-card'}`}
              onClick={() => setActiveState('Base State')}
            >
              <div className="truncate flex-1 font-medium">Base State</div>
              <div className="text-ios-blue">
                {active === 'Base State' && <Check className="h-4 w-4" />}
              </div>
            </button>
            {states.map((s) => (
              <button
                key={s}
                type="button"
                className={`ios-list-item w-full text-left transition-all ios-active-scale ${active === s ? 'bg-muted' : 'bg-card'}`}
                onClick={() => setActiveState(s as any)}
              >
                <div className="truncate flex-1 font-medium">{s}</div>
                <div className="text-ios-blue">
                  {active === s && <Check className="h-4 w-4" />}
                </div>
              </button>
            ))}
          </div>
